...
(defun update-broad-phase-box (box)
  ...
  (v3:with-components ((v (collision-volume-box-half-extents box)))
    (let* ((owner (trait-owner (collision-volume-collider box)))
           (center (collision-volume-center box))
           (rotation (collision-volume-box-axes box))
           (p1 (v3:copy v))
           (p2 (v3:vec (- vx) vy vz))
           (p3 (v3:vec vx (- vy) vz))
           (p4 (v3:vec vx vy (- vz)))
      ...
      (v3:+! p7 center p7)
      (v3:+! p8 center p8)
      (transform-point! owner p1 p1)
      (transform-point! owner p2 p2)
      (transform-point! owner p3 p3)
      (transform-point! owner p4 p4)
      (transform-point! owner p5 p5)
      (transform-point! owner p6 p6)
      (transform-point! owner p7 p7)
      (transform-point! owner p8 p8)
