...
(defun update-collision-plan (name multi-level-p cell-size bucket-size mappings)
  (let* ((data (u:href =collision-plans= name))
         (table (collision-plan-table data)))
    (setf (collision-plan-multi-level-p data) multi-level-p
          (collision-plan-cell-size data) cell-size
...
(defun make-collision-plan (name multi-level-p cell-size bucket-size mappings)
  (let ((plan (%make-collision-plan :name name)))
    ...
    (update-collision-plan name multi-level-p cell-size bucket-size mappings)
    plan))

(defmacro define-collision-plan (name (&key (bucket-size 1024) (multi-level-p t) (cell-size 128))
                                 &body body)
  `(if (u:href =collision-plans= ',name)
       (update-collision-plan ',name ,multi-level-p ,cell-size ,bucket-size ',body)
       (make-collision-plan ',name ,multi-level-p ,cell-size ,bucket-size ',body)))
