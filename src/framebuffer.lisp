  ...
  (mapcar
   (lambda (x)
     (let* ((data (data framebuffer))
            (attachment (data::find-attachment data x)))
       (unless attachment
         (error "Attachment name ~s does not exist for framebuffer ~s." x (data::name data)))
       (point->gl (data::attachment-point attachment))))
   attachment-names))
  ...
  (let ((result (gl:check-framebuffer-status target)))
    (unless (cl:find result '(:framebuffer-complete :framebuffer-complete-oes))
      (error "Framebuffer not complete: ~a~%~%Framebuffer: ~s~%Buffer: ~a~%Attachment point: ~s"
             result
             (data::name (data framebuffer))
             buffer
             attachment-point))
    nil))

(u:fn-> attach-render-buffer (ctx::context framebuffer data::attachment) u:ub32)
(declaim (inline attach-render-buffer))
(defun attach-render-buffer (context framebuffer attachment)
  (declare (optimize speed))
  (let* ((point (data::attachment-point attachment))
         (gl-point (point->gl point))
         (format (point->render-buffer-format point))
         (buffer-id (gl:gen-renderbuffer))
         (window (ctx::window context))
         (width (funcall (data::attachment-width attachment) window))
         (height (funcall (data::attachment-height attachment) window))
...
(u:fn-> attach-texture (ctx::context framebuffer data::attachment) u:ub32)
(declaim (inline attach-texture))
(defun attach-texture (context framebuffer attachment)
  (declare (optimize speed))
  (let ((attachment-name (data::attachment-name attachment))
        (point (data::attachment-point attachment))
        (buffer (data::attachment-buffer attachment))
        (width-func (data::attachment-width attachment))
        (height-func (data::attachment-height attachment)))
...
(defun attach (context framebuffer attachment-name)
  (declare (optimize speed))
  (let ((attachment (data::find-attachment (data framebuffer) attachment-name)))
    (ecase (car (data::attachment-buffer attachment))
...
(defun attach-all (context framebuffer)
  (declare (optimize speed))
  (u:do-hash-values (attachment (data::attachments (data framebuffer)))
    (attach context framebuffer (data::attachment-name attachment))))

(u:fn-> make-framebuffer (ctx::context data::framebuffer) framebuffer)
(defun make-framebuffer (context data)
  (declare (optimize speed))
  (let ((framebuffer (%make-framebuffer :data data
                                        :id (gl:gen-framebuffer)
                                        :target (mode->target (data::mode data)))))
    (attach-all context framebuffer)
    (setf (u:href (ctx::framebuffers context) (data::name data)) framebuffer)
    framebuffer))
...
(defun load (context name)
  ...
  (or (find context name)
      (make-framebuffer context (data::find name))))
