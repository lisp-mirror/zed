...
(defun make-material (context type)
  (declare (optimize speed))
  (let* ((materials (context-materials context))
         (data (find-material-data type))
...
(u:fn-> ensure-material (context symbol) material)
(defun ensure-material (context type)
  (declare (optimize speed))
  (or (u:href (context-materials context) type)
...
(defmethod recompile ((type (eql :material)) data)
  ...
  (u:when-let ((material (u:href (context-materials =context=) data)))
    (make-uniforms =context= material)
    (ensure-framebuffer =context= material))
