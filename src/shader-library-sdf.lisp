...
(defun sdf/onion ((d :float)
                  (r :float))
  (- (abs d) r))

(defun sdf/union ((d1 :float)
                  (d2 :float))
  (min d1 d2))

(defun sdf/difference ((d1 :float)
                       (d2 :float))
  (max (- d1) d2))

(defun sdf/intersect ((d1 :float)
                      (d2 :float))
  (max d1 d2))

(defun sdf/union-smooth ((d1 :float)
                         (d2 :float)
                         (k :float))
  (let ((h (saturate (+ 0.5 (/ (* 0.5 (- d2 d1)) k)))))
    (- (mix d2 d1 h) (* k h (- 1 h)))))

(defun sdf/difference-smooth ((d1 :float)
                              (d2 :float)
                              (k :float))
  (let ((h (saturate (- 0.5 (/ (* 0.5 (+ d2 d1)) k)))))
    (+ (mix d2 (- d1) h) (* k h (- 1 h)))))

(defun sdf/intersect-smooth ((d1 :float)
                             (d2 :float)
                             (k :float))
  (let ((h (saturate (- 0.5 (/ (* 0.5 (- d2 d1)) k)))))
    (+ (mix d2 d1 h) (* k h (- 1 h)))))

(defun sdf/mask-fill ((dist :float))
  (saturate (- dist)))

(defun sdf/mask-inner-border ((dist :float)
                              (width :float))
  (- (saturate (+ dist width))
     (saturate dist)))

(defun sdf/mask-outer-border ((dist :float)
                              (width :float))
  (- (saturate dist)
     (saturate (- dist width))))

(defun sdf/shadow ((fn (function (:vec2) :float))
                   (p :vec2)
                   (pos :vec2)
                   (radius :float))
...
(defun sdf/circle ((p :vec2)
                   (r :float))
  (- (length p) r))

(defun sdf/box ((p :vec2)
                (b :vec2))
  (let ((d (- (abs p) b)))
    (+ (length (max d 0))
       (min (max (.x d) (.y d)) 0))))

(defun sdf/segment ((p :vec2)
                    (a :vec2)
                    (b :vec2))
  (let* ((pa (- p a))
         (ba (- b a))
...
(defun sdf/semi-circle ((p :vec2)
                        (radius :float)
                        (angle :float)
                        (width :float))
  (let* ((width (/ width 2))
         (radius (- radius width)))
    (sdf/difference (sdf/pie p angle)
                    (- (abs (sdf/circle p radius)) width))))

(defun sdf/uneven-capsule ((p :vec2)
                           (r1 :float)
                           (r2 :float)
                           (h :float))
  (let* ((p (vec2 (abs (.x p)) (.y p)))
...
(defun sdf/rounded-x ((p :vec2)
                      (w :float)
                      (r :float))
  (let ((p (abs p)))
    (- (length (- p (* (min (+ (.x p) (.y p)) w) 0.5))) r)))

(defun sdf/parabola ((pos :vec2)
                     (k :float))
  (let* ((pos (abs pos))
         (ik (/ k))
         (p (* ik (/ (- (.y pos) (* 0.5 ik)) 3)))
