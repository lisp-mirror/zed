...
(u:fn-> on-button-enter (core &rest t) boolean)
(declaim (inline on-button-enter))
(defun on-button-enter (core &rest args)
  (declare (optimize speed))
  (u:when-let* ((manager (core-input-manager core))
                (state (u:href (input-manager-states manager) args)))
    (input-transition-enter state)))

(u:fn-> on-button-enabled (core &rest t) boolean)
(declaim (inline on-button-enabled))
(defun on-button-enabled (core &rest args)
  (declare (optimize speed))
  (u:when-let* ((manager (core-input-manager core))
                (state (u:href (input-manager-states manager) args)))
    (input-transition-enabled state)))

(u:fn-> on-button-exit (core &rest t) boolean)
(declaim (inline on-button-exit))
(defun on-button-exit (core &rest args)
  (declare (optimize speed))
  (u:when-let* ((manager (core-input-manager core))
                (state (u:href (input-manager-states manager) args)))
