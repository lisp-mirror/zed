...
(defpackage #:%zed.shader.swizzle
  ;; Third-party aliases
  (:local-nicknames
   (#:u #:golden-utils))
  (:use #:cl))

(in-package #:%zed.shader.swizzle)
...
(u:eval-always
  ...
  (defmacro generate-swizzle-operators ()
    (flet ((map-swizzle (mask)
      ...
      `(progn
         ,@(loop :for mask :in (%swizzle/component-groups 4)
                 ...
                 :collect `(export ',op)
                 :collect `(vari:v-defmacro ,op (vector)
                             `(vari:swizzle ,vector ,,(map-swizzle mask))))))))
