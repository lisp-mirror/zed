...
(defun attach (render)
  (declare (optimize speed))
  (let* ((owner (trait::owner render))
         (found (trait:find-trait owner 'render)))
    (when (and found (not (eq found render)))
      (error "A game object can have at most 1 render trait attached."))
