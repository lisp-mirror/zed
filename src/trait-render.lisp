...
(defun setup (render)
  (declare (optimize speed))
  (let ((viewport-manager (z::context-viewports (z:trait-context render))))
...
(defun attach (render)
  ...
  (u:if-let ((material-name (material-name render)))
    (let ((context (z:trait-context render)))
      (setf (material render) (z::make-material context material-name))
...
(defun detach (render)
  (declare (optimize speed))
  (let ((context (z:trait-context render)))
    (z::destroy-material context (material render))
