...
(defmethod tex::load-source (context data (type (eql :2d-array)) source &key width height)
  (declare (optimize speed))
  (cond
    ((null source)
     (list (tex::load-framebuffer-texture context data width height)))
    ...
    ((and (typep source 'u:proper-list)
          (every #'listp source))
     (lp:pmapcar (lambda (x) (img::load context x)) source))
