...
(u:fn-> resolve-uniform-value (context game-object uniform) t)
(defun resolve-uniform-value (context game-object uniform)
...
(u:fn-> resolve-uniform-function (context game-object uniform) t)
(defun resolve-uniform-function (context game-object uniform)
  (funcall (uniform-func uniform)
           (uniform-program uniform)
           (uniform-key uniform)
           (resolve-uniform-value context game-object uniform)))

(u:fn-> load-uniform-texture (context material uniform) null)
(defun load-uniform-texture (context material uniform)
  (declare (optimize speed))
  (let ((value (uniform-value uniform)))
    (when (eq (uniform-resolved-type uniform) :sampler)
      (let ((material-name (material-data-name (material-data material)))
            (texture (load-texture context value)))
  ...
  (let ((material-data (material-data material)))
    ...
    (u:do-hash (k v (copy-material-data-uniforms material-data))
      (let ((uniform (make-uniform :key k :value v)))
        ...
        (load-uniform-texture context material uniform)
