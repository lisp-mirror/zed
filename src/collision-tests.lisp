...
(defmethod collide-p ((volume1 collision-volume-sphere)
  ...
  (let ((game-object1 (trait-owner (collision-volume-collider volume1)))
        (game-object2 (trait-owner (collision-volume-collider volume2))))
    (<= (p3:distance
         (transform-point game-object1 (collision-volume-center volume1))
         (transform-point game-object2 (collision-volume-center volume2)))
        (+ (v3:length (transform-vector game-object1 v3:+right+))
           (v3:length (transform-vector game-object2 v3:+right+))))))
