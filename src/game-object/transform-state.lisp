...
(defpackage #:%zed.game-object.transform-state
  ;; Third-party aliases
  (:local-nicknames
   (#:m4 #:origin.mat4)
   (#:q #:origin.quat)
...
(in-package #:%zed.game-object.transform-state)
...
(u:fn-> transform-node/translation (state u:f32) null)
(declaim (inline transform-node/translation))
(defun transform-node/translation (state delta)
  (declare (optimize speed))
...
(u:fn-> transform-node/rotation (state u:f32) null)
(declaim (inline transform-node/rotation))
(defun transform-node/rotation (state delta)
  (declare (optimize speed))
...
(u:fn-> transform-node/scale (state u:f32) null)
(declaim (inline transform-node/scale))
(defun transform-node/scale (state delta)
  (declare (optimize speed))
...
(u:fn-> transform-node (state u:f32) null)
(defun transform-node (state delta)
  (declare (optimize speed))
  (transform-node/translation state delta)
  (transform-node/rotation state delta)
  (transform-node/scale state delta)
  nil)
