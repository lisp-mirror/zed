...
(defun map-font-glyphs (font-spec func string)
  (loop :with w = (float (font:scale-w font-spec) 1f0)
        ...
        :do (case char
              ...
              (t
               ...
               (let ((x- (+ x (getf char-data :xoffset)))
                     ...
                     (x+ (+ x (getf char-data :xoffset) (getf char-data :width)))
                     (y+ (+ y (getf char-data :yoffset) (getf char-data :height)))
                     (u- (/ (getf char-data :x) w))
                     (v- (/ (getf char-data :y) h))
                     (u+ (/ (+ (getf char-data :x) (getf char-data :width)) w))
                     (v+ (/ (+ (getf char-data :y) (getf char-data :height)) h)))
        ...
        :finally (funcall func vertex-data)
                 (return (values (float (/ (max x (apply #'max line-widths)) 2) 1f0)
                                 (float (/ y 2) 1f0)))))
