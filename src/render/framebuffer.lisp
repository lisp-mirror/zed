...
(u:fn-> find-framebuffer (core symbol) (or framebuffer null))
(declaim (inline find-framebuffer))
(defun find-framebuffer (core name)
  (declare (optimize speed))
  (u:href (core-framebuffers core) name))
...
(u:fn-> framebuffer-attach-render-buffer (core framebuffer framebuffer-attachment) u:ub32)
...
(u:fn-> framebuffer-attach-texture (core framebuffer framebuffer-attachment) u:ub32)
(declaim (inline attach-framebuffer-texture))
(defun attach-framebuffer-texture (core framebuffer attachment)
  (declare (optimize speed))
...
(u:fn-> framebuffer-attach (core framebuffer symbol) u:ub32)
(defun framebuffer-attach (core framebuffer attachment-name)
  ...
  (let ((attachment (find-framebuffer-attachment (framebuffer-data framebuffer) attachment-name)))
    (ecase (car (framebuffer-attachment-buffer attachment))
      (:render-buffer (framebuffer-attach-render-buffer core framebuffer attachment))
      (:texture (framebuffer-attach-texture core framebuffer attachment)))))

(u:fn-> framebuffer-attach-all (core framebuffer) null)
(defun framebuffer-attach-all (core framebuffer)
  ...
  (u:do-hash-values (attachment (framebuffer-data-attachments (framebuffer-data framebuffer)))
    (framebuffer-attach core framebuffer (framebuffer-attachment-name attachment))))

(u:fn-> make-framebuffer (core framebuffer-data) framebuffer)
(defun make-framebuffer (core data)
  (declare (optimize speed))
  (let ((framebuffer (%make-framebuffer
    ...
    (setf (u:href (core-framebuffers core) (framebuffer-data-name data)) framebuffer)
    framebuffer))

(u:fn-> load-framebuffer (core symbol) framebuffer)
(declaim (inline load-framebuffer))
(defun load-framebuffer (core name)
  (declare (optimize speed))
  (or (find-framebuffer core name)
      (make-framebuffer core (find-framebuffer-data name))))
